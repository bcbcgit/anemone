{{-- resources/views/kinds/index.blade.php --}}
@extends('layouts.admin')

@section('title', 'シナリオ種別 管理')
@section('page_title', 'シナリオ種別 管理')
...
@section('content')
    <div class="max-w-3xl mx-auto space-y-6">
        ...
        {{-- 登録フォーム（index内） --}}
        <form action="{{ route('kinds.store') }}" method="post" class="space-y-4">
            @csrf
            <div class="rounded-md border border-slate-200 bg-white p-4 shadow-sm space-y-3">
                <div>
                    <label class="block text-sm font-medium text-slate-700">
                        シナリオ種別 <span class="text-rose-600">*</span>
                    </label>
            ...
            <table class="w-full text-sm">
                ...
                @foreach ($kinds as $k)
                    <tr class="border-t border-slate-200"
                        data-id="{{ $k->id }}"
                        data-update-url="{{ route('kinds.update', $k) }}"
                        data-destroy-url="{{ route('kinds.destroy', $k) }}"
    ...
    <script>
        (function(){
            ...
            const destroyRow = async (tr) => {
                if (!confirm('このシナリオ種別を削除しますか？')) return;
    ...
    <script>
        (function () {
            ...
            // 4) 値の取り方（IDはテキスト、種別は input の値を優先）
            const getter = (tr, key) => {
