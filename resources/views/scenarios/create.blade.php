        ...
        <div class="rounded-md border border-slate-200 bg-white p-4 shadow-sm space-y-6">
            <div>
                <div class="flex items-center justify-between">
                    ...
                    <div class="flex gap-2">
                        <button type="button" class="rounded-md border border-slate-200 px-2 py-1 text-xs"
                                data-select-all="elements">全選択</button>
                        <button type="button" class="rounded-md border border-slate-200 px-2 py-1 text-xs"
                                data-clear="elements">選択解除</button>
                    </div>
                </div>
                ...
                @php
                    $oldElementIds = collect(old('elements', []))->map(fn($v)=>(string)$v)->all();
                @endphp

                <div id="elementsContainer" class="mt-3 flex flex-wrap gap-3">
                    @forelse ($elements as $element)
                        @php $checked = in_array((string)$element->id, $oldElementIds, true); @endphp
                        <div class="flex items-center gap-2 px-2 py-1 rounded-md ring-1 ring-slate-200 bg-slate-50" data-kind-id="{{ $element->id }}">
                            <label class="inline-flex items-center gap-2 text-sm">
                                <input type="checkbox" name="elements[]"
                                       class="rounded-md border border-slate-400 focus:border-slate-600 focus:ring-slate-500"
                                       value="{{ $element->id }}" @checked($checked)>
                                <span class="select-none">{{ $element->title }}</span>
                            </label>
                        </div>
                    @empty
                        <p class="text-sm text-slate-500">選択できるシナリオ種別がありません。</p>
